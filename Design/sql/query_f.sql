SELECT use.user_id, rest.restaurant_id, use.name, rest.name, COUNT(rate.post_date), AVG((rate.food+rate.price+rate.mood+rate.staff)/4.0)
	...
	GROUP BY use.user_id, rest.restaurant_id, rest.name, use.name
