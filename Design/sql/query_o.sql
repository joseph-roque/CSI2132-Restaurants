SELECT use.name, use.user_id, rt.description, use.email, loc.location_id, rest.name, MAX(temp.rateTotal) rate_max, MIN(temp.rateTotal) rate_min, (MAX(temp.rateTotal) - MIN(temp.rateTotal)) rate_diff, ct.description
	FROM Rater use
	...
	GROUP BY use.name, use.user_id, rt.description, use.email, loc.location_id, rest.name, ct.description
